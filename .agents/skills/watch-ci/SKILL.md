...
7. Investigate a real failure (a judgment step):
   ...
   3. Reproduce locally with the command for that area in the file named by
      `VIVI_TEST_COMMANDS_FILE` in `repo.env` (`bin/build` runs all of them). If it passes locally and failed in CI
