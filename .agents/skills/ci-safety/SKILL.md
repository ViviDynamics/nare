---
name: ci-safety
description: Use when reading CI state, deciding whether a CI failure is a flake, retrying a GitHub Actions run, or writing any poll loop over CI. Background rules for watch-ci and merge-pr.
license: Elastic-2.0
...
| `known_flake` | log matched a line in this repo's flake signatures file | first time on this PR: verified retry. Second time, same signature, same PR: stop and root-cause |
